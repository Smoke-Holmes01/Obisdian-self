...
> [!NOTE] **Mock 测试技术**
> Mock 是在测试过程中，对于一些不容易构造/获取的对象，用一个虚拟的对象来替代它，以简化测试的测试技术。
> - **SUT**：System Under Test (被测系统) —— 我们真正关注的被测对象。
> - **DOC**：Depend-on Component (依赖组件) —— 通过 Mock 机制进行隔离，使测试专注于 SUT 本身。
> - **Round-trip test**：直接输入并通过公开接口验证的回路测试。
...
#### API的TA架构
> [!TIP] **API 自动化测试架构 (Test Automation Architecture)**
> - **API定义**：如 **RAML**（RESTful API描述性语言）、**HAR**（基于JSON的HTTP存档文件格式）。
> - **API的来源**：文档化工具、手工录入、程序扫描、HAR导入、Swagger、Markdown。
...
#### Airtest 框架
![](img/Ch9_图片3.jpg)
> [!NOTE] **Airtest 自动化测试框架核心组件**
> Airtest 是由网易开源的跨平台 UI 自动化测试框架，支持 Python 语言，包含三大核心组件：
> - **Airtest**：跨平台的基于**图像识别**的 UI 自动化测试框架。
> - **Poco**：基于**UI 控件识别**的自动化测试框架，支持 Android、iOS 原生应用和微信小程序。
> - **AirtestIDE**：跨平台的集成开发环境，提供脚本录制、一键回放与测试报告查看等功能。
